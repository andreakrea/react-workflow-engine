...
function WorkflowCanvas({ apiUrl, blockTypes, hookTypes, onBack, title }: CanvasProps) {
  ...
  const renameWorkflow = async (workflowId: number, currentName: string, e: React.MouseEvent) => {
    e.stopPropagation();
    const newName = prompt('Rename workflow:', currentName);
    if (!newName || newName.trim() === '' || newName === currentName) return;
    try {
      const res = await fetch(apiRoute(`/api/workflows/${workflowId}`), {
        method: 'PUT',
      ...
      console.error('WorkflowEditor: error renaming workflow', err);
    }
  };
                  ...
                  >
                    <div className="flex items-start justify-between">
                      ...
                      <div className="flex items-center ml-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button
                          onClick={e => renameWorkflow(workflow.id, workflow.name, e)}
